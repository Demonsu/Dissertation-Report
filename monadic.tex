...
While I was implementing this library, I have encountered several problems. Most of them were solved by other teammates in the group (HKU Programming Language Group), but still have some problems has to be solved or to be improved.
...
It would be better if just separate those codes into different files and then combine them together before compile. To achieve that goal, I wrote a script \texttt{include.py} in Python to perform C-like include mechanism.
...
So I use F2J to write a simple test framework, supporting test functions, test suites and assert functions.
...
An example in \ref{fig:monadic_runalltests} shows how the test framework runs.
