...
The \texttt{<|>} operator represents a union of two layouts. The \texttt{flatten} operator deletes all the line breaks in a layout. Then the document now denotes a set of possible layouts, and all the layouts in it will be the same after \texttt{flatten}. With \texttt{<|>} and \texttt{flatten}, group can be defined as:
\begin{lstlisting}
let (<|>) (x: Doc) (y: Doc) : Doc =
    UNION x y
;
